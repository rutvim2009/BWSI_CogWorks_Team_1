...
def propagate_label(node, neighbors, adjacency_matrix): #Weigh the neighbors based on the adjacency matrix, change the current node's label to that with the highest wight
        if not neighbors:
            return
    
        pairs = []
        for neighbor in neighbors:
            weight = adjacency_matrix[node.id][neighbor.id]
            pairs.append((weight, neighbor.label))
...
def whispers(nodes, adjacency_matrix, num_iterations=100, plot_convergence=True):
    ...
    for i in range(len(nodes)):
        ...
        neighbor_lookup.append(neighbors)
    ...
    for step in range(1, num_iterations + 1):
        ...
        neighbors = neighbor_lookup[random_node.id]
    ...
    if plot_convergence:
        ...
        plt.plot(hist_iter, hist_comp_counts, marker='o', color='b')
        plt.title("whispers convergence")
        plt.xlabel("iterations")
        plt.ylabel("num connected components/clusters")
